...
A ordem segue o grafo de dependências — cada camada só usa o que já foi construído:


1. config.py
   └── base de tudo, sem dependências

2. observability/tracer.py
   └── instrumenta desde o início — assim cada etapa seguinte já é rastreada

3. memory/short_term.py
   └── só Python puro, sem deps externas

4. memory/long_term.py
   └── SQLite — depende só do config (DB_PATH)

5. guardrails/input_validator.py
   └── regex puro, sem deps

6. guardrails/output_validator.py
   └── depende só do config (MAX_OUTPUT_CHARS)

7. rag/hybrid_rag.py
   └── depende do config + sentence-transformers + rank-bm25
   └── aqui você testa: "a busca está achando os trechos certos?"

8. agents/researcher.py
   └── primeiro agente funcional — junta RAG + LLM + tracer
   └── já dá pra testar perguntas sobre os docs

9. agents/sql_agent.py
   └── segundo agente — function calling loop + MotherDuck
   └── testa independente do researcher

10. agents/reviewer.py
    └── terceiro agente — adiciona streaming
    └── recebe resultado de qualquer agente e refina

11. agents/orchestrator.py
    └── junta tudo: guardrails + memória + agentes + revisor
    └── primeiro momento que o fluxo completo funciona de ponta a ponta

12. evals/llm_judge.py + cases.json
    └── agora que o fluxo funciona, mede a qualidade
    └── roda os casos e vê onde está errando

13. interfaces/cli.py
    └── empacota o orquestrador numa interface usável

14. main.py
    └── entry point — só organiza os comandos
Por que essa ordem?
Etapa	Razão
Config antes de tudo	Evita hardcode em qualquer módulo
Tracer cedo	Cada chamada LLM já é rastreada desde o início
Memória antes dos agentes	Orquestrador precisa dela
Guardrails antes do orquestrador	Orquestrador os chama
RAG antes do researcher	Researcher depende do RAG
Agentes antes do orquestrador	Orquestrador os chama
Evals depois do fluxo completo	Precisa de respostas reais para avaliar
CLI por último	Só empacota o que já funciona
Cada passo tem algo testável — você não precisa esperar o projeto inteiro para ver funcionar.
```
